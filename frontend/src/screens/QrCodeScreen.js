...
import "../assets/css/qrscreen.css";
import { Spinner } from "../components/Spinner";

import { MainWrapper } from "../components/Global/MainWrapper";
import io from "socket.io-client";
import { HorizontalRule } from "components/Global/HorizontalRule";
...
const QrCodeScreen = () => {
    ...
    useEffect(() => {
        ...
        socket.current.emit("requestQr");

        socket.current.on("get_qr", (data) => {
            console.log(data);
            setQr(data.qr);
        });
        return () => {
        ...
        <MainWrapper>
            <div className="barCode">
                <div className="barCode__content">
                    <div className="barCode__content__row">
                        <div className="barCode__content__row__description">
                            <h2>
                                <span className="orange_txt">Scan to connect with whatsapp</span>
                                <br /> thorugh auto advert
                            </h2>
                            <ol>
                                <li>
                                    Open Whatsapp on your phone. <span className="_30yMe"></span>
                                </li>
                                <li>
                                    Tap Menu{" "}
                        ...
                        <div className="qrcode">
                            {qr === "" ? (
                                <>
                                    {" "}
                                    <div style={{ margin: "70px 0 10px 100px" }}>
                                        <Spinner size={6} />
                                    </div>
                                    <p style={{ textAlign: "center" }}>Loading Qrcode ...</p>
                                </>
                            ) : (
                                <div className="qrBarCode">
                                    <QRCodeSVG value={qr} size={270} />
                                    <HorizontalRule />
